...
export default function CoffeeMenu() {
  ...
  useEffect(() => {
    ...
    const fetchDrinks = async () => {
      try {
        const res = await fetch('/api/drinks?category=' + encodeURIComponent('Бабл кофе'));
  ...
  useEffect(() => {
    const onScroll = () => { try { sessionStorage.setItem('menu_coffee_scroll', String(window.scrollY)); } catch {} };
  ...
  useEffect(() => {
    if (isLoading) return;
    try {
      const saved = sessionStorage.getItem('menu_coffee_scroll');
