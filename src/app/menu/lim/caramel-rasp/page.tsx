...
export default function LimonadeTemplatePage() {
  // === СТЕЙТЫ КОРЗИНЫ ===
  ...
  const pickupPrice = 300;   // Цена за M Самовывоз (меняй на свою)
  const deliveryPrice = 420; // Цена за M Доставка (меняй на свою)
  const basePrice = orderType === 'delivery' ? deliveryPrice : pickupPrice;

  // ❗❗❗ 2. ТУТ ПИШЕШЬ ID И ИМЯ ❗❗❗
  const productId = 'caramel-rasp'; 
  const productName = 'Карамельная малина';       
  const productImg = '/images/caramel-rasp1.jpg'; // ❗❗❗ 3. ФОТКА ДЛЯ КОРЗИНЫ ❗❗❗
  ...
  // Стейт для выпадающего списка Джус-боллов и выбранных вкусов
  ...
  if (selectedVolume === 'L') finalPrice += 50; // Наценка за объем L
  if (tapiocaX2Selected || juiceX2Selected) finalPrice += 80; // Наценка за Х2

  // Для лимонадов тип жестко "Холодный"
    ...
    <div className="bg-[#FDFDFD] min-h-[100dvh] w-full flex justify-center overflow-y-auto overflow-x-hidden font-sans">
      <main className="w-full max-w-[370px] relative flex flex-col items-center pb-[120px]">
        ...
        <div className="relative w-[370px] h-[360px] shrink-0">
          ...
          <Image src="/images/caramel-rasp1.jpg" alt="Карамельная малина" fill quality={100} className="object-cover" priority />
        ...
        <div className="w-full px-[12px] mt-[16px] mb-[16px] z-20">
          <h1 className="text-[19px] font-black uppercase tracking-[0.02em] bg-gradient-to-r from-[#FF00EE] to-[#FF008C] text-transparent bg-clip-text leading-none"
        ...
        <div className="w-[346px] h-[150px] mb-[32px] bg-[#EEEEEE] border border-[#FFFFFF]/40 shadow-[0px_5px_5.7px_4px_rgba(255,0,140,0.25)] rounded-[25px] flex items-center justify-center z-20 backdrop-blur-[30px] box-border shrink-0">
          <div className="w-[322px] h-[130px] box-border overflow-y-auto no-scrollbar">
            <p className="text-[12px] text-[#272727] leading-[1.4] text-justify" style={{ fontFamily: "'Benzin-Regular', sans-serif" }}>
              ...
              Сладкая малина и нежная карамель танцуют в игривой содовой, создавая яркий ягодно-карамельный дуэт. Летняя сладость переплетается с тёплыми карамельными нотами в каждом глотке. А завершает эту историю текстурная игра на выбор: упругие жемчужины тапиоки или взрывные джус-боллы
            </p>
          </div>
        </div>
        ...
        >
          ...
          <div className="w-full h-[52px] flex justify-between items-center shrink-0 cursor-pointer px-[16px]" onClick={() => setIsAddonsOpen(!isAddonsOpen)}>
            <div className="flex items-center justify-start">
              <span className="text-[18px] tracking-[0.02em] whitespace-nowrap bg-gradient-to-r from-[#FF00EE] to-[#FF008C] bg-clip-text text-transparent leading-none" style={{ fontFamily: "'Benzin', sans-serif", fontWeight: 800 }}>
                Дополнения
              </span>
            </div>
          ...
          <div className="w-[314px] h-fit pb-[16px] mt-[32px] bg-[#AEAEAE]/20 rounded-[25px] flex flex-col box-border shrink-0 transition-all duration-500 overflow-hidden" style={{ boxShadow: 'inset 0px 0px 0px 1px rgba(255, 255, 255, 0.4), 0px 5px 5.7px 4px rgba(255, 0, 140, 0.25)' }}>
            ...
            <div className="w-full flex justify-between items-center mt-[12px] px-[16px] shrink-0 cursor-pointer" onClick={() => setIsJuiceOpen(!isJuiceOpen)}>
              <span className="text-[16px] tracking-[0.02em] whitespace-nowrap bg-gradient-to-r from-[#FF00EE] to-[#FF008C] bg-clip-text text-transparent leading-none block" style={{ fontFamily: "'Benzin', sans-serif", fontWeight: 800 }}>
                Джус боллы
              </span>
              <div className="w-[22px] h-[22px] shrink-0 flex items-center justify-center">
                <svg width="8" height="14" viewBox="0 0 8 14" fill="none" xmlns="http://www.w3.org/2000/svg" className={`transition-transform duration-300 ${isJuiceOpen ? 'rotate-90' : 'rotate-0'}`}>
                  <path d="M1 1.5L6.5 7L1 12.5" stroke="#949494" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </div>
            </div>
            ...
            <div className={`w-full flex flex-col overflow-hidden transition-all duration-500 ease-in-out ${isJuiceOpen ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'}`}>
              {juiceFlavorsList.map((flavor, index) => (
                <div key={flavor} className={`w-full flex justify-between items-center px-[24px] shrink-0 ${index === 0 ? 'mt-[24px]' : 'mt-[16px]'}`}>
                  <span className="text-[16px] tracking-[0.02em] whitespace-nowrap bg-gradient-to-r from-[#FF00EE] to-[#FF008C] bg-clip-text text-transparent leading-none block" style={{ fontFamily: "'Benzin', sans-serif", fontWeight: 800 }}>
                    {flavor}
                  </span>
                  <div className="flex items-center gap-[10px] shrink-0">
                    <div onClick={() => handleJuiceFlavorClick(flavor)} className={`w-[22px] h-[22px] rounded-[6px] border cursor-pointer flex items-center justify-center transition-all duration-300 shrink-0 ${selectedJuiceFlavors.includes(flavor) ? 'bg-[#FF008C] border-[#FF008C] shadow-[0_0_10px_rgba(255,0,140,0.5)]' : 'border-[#949494] bg-transparent'}`}>
                      {selectedJuiceFlavors.includes(flavor) && <CheckMark />}
                    </div>
                  </div>
                </div>
              ))}
          ...
          {/* КРАФТИНГ */}
          <div className="w-[314px] h-[154px] mt-[32px] bg-[#AEAEAE]/20 rounded-[25px] flex flex-col box-border shrink-0" style={{ boxShadow: 'inset 0px 0px 0px 1px rgba(255, 255, 255, 0.4), 0px 5px 5.7px 4px rgba(255, 0, 140, 0.25)' }}>
            <div className="mt-[16px] ml-[16px] shrink-0">
            ...
            <div className={`w-full flex justify-between items-center mt-[16px] px-[16px] shrink-0 transition-opacity duration-300 ${tapiocaSelected ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
              ...
              <div className="flex items-center gap-[10px] shrink-0">
                 <span className={`text-[12px] text-[#FF008C] whitespace-nowrap transition-all duration-300 ${tapiocaX2Selected ? 'opacity-100' : 'opacity-0'}`} style={{ fontFamily: "'Benzin', sans-serif", fontWeight: 800 }}>
                   {/* ❗❗❗ МЕНЯТЬ ЦЕНУ ТАПИОКИ X2 ТУТ ❗❗❗ */}
                   + 80 ₽
                 </span>
            ...
            <div className={`w-full flex justify-between items-center mt-[12px] px-[16px] shrink-0 transition-opacity duration-300 ${juiceSelected ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
              <span className="text-[16px] tracking-[0.02em] whitespace-nowrap bg-gradient-to-r from-[#FF00EE] to-[#FF008C] bg-clip-text text-transparent leading-none block uppercase" style={{ fontFamily: "'Benzin', sans-serif", fontWeight: 800 }}>Джус боллы 2X</span>
              <div className="flex items-center gap-[10px] shrink-0">
                <span className={`text-[12px] text-[#FF008C] whitespace-nowrap transition-all duration-300 ${juiceX2Selected ? 'opacity-100' : 'opacity-0'}`} style={{ fontFamily: "'Benzin', sans-serif", fontWeight: 800 }}>
                  {/* ❗❗❗ МЕНЯТЬ ЦЕНУ ДЖУС БОЛЛОВ X2 ТУТ ❗❗❗ */}
                  + 80 ₽
                </span>
                <div onClick={handleJuiceX2Click} className={`w-[22px] h-[22px] rounded-[6px] border cursor-pointer flex items-center justify-center transition-all duration-300 shrink-0 ${juiceX2Selected ? 'bg-[#FF008C] border-[#FF008C] shadow-[0_0_10px_rgba(255,0,140,0.5)]' : 'border-[#949494] bg-transparent'}`}>
                  {juiceX2Selected && <CheckMark />}
                </div>
              </div>
            </div>
          </div>
        ...
        {/* === 5. УМНАЯ КНОПКА (С РЕШЕНИЕМ РАССИНХРОНА) === */}
