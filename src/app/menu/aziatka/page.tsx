...
export default function AziatkaMenu() {
  ...
  useEffect(() => {
    ...
    const processData = (data: any[]) => {
      const formatted = data.map(d => {
        ...
        return {
          id: slug, name: d.name,
          pickupPrice: d.price_pickup, deliveryPrice: d.price_delivery,
          img: `/images/aziatka/${slug}.jpg`, href: `/menu/aziatka/${slug}`,
        };
      });
      setItems(formatted);
    };
    try {
      const cached = localStorage.getItem('menu_aziatka_cache');
      if (cached) { processData(JSON.parse(cached)); setIsLoading(false); }
    } catch {}
    const fetchItems = async () => {
      try {
        ...
        processData(data);
        try { localStorage.setItem('menu_aziatka_cache', JSON.stringify(data)); } catch {}
      } catch {}
      setIsLoading(false);
    };
    fetchItems();
  ...
  useEffect(() => {
    const onScroll = () => { try { sessionStorage.setItem('menu_aziatka_scroll', String(window.scrollY)); } catch {} };
    window.addEventListener('scroll', onScroll);
    return () => window.removeEventListener('scroll', onScroll);
  }, []);
  useEffect(() => {
    if (isLoading || !isBoss) return;
    try {
      const saved = sessionStorage.getItem('menu_aziatka_scroll');
  ...
  if (!isBoss) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-[#FAFAFA]">
        ...
        <Link href="/" className="h-[46px] px-[24px] bg-gradient-to-r from-[#FF00EE] to-[#FF008C] text-white rounded-[14px] font-['Benzin'] font-extrabold text-[12px] uppercase active:scale-95 flex items-center">
          На главную
        </Link>
      </div>
    );
    ...
    <div className="p-4 pb-[180px] flex flex-col items-center bg-[#FAFAFA] min-h-screen">
      <div className="w-full max-w-[370px] flex items-center justify-between mb-6 mt-2 relative h-[40px]">
        ...
        <Link href="/" className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 active:scale-95 transition-transform flex justify-center items-center">
          <Image draggable={false} src="/images/logo.png" alt="Логотип" width={160} height={45} quality={100} className="object-contain" />
        </Link>
        <Link href="/profile" className="w-[34px] h-[34px] rounded-full overflow-hidden shrink-0 shadow-sm active:scale-90 transition-transform bg-[#E5E5EA]">
          <Image draggable={false} src={avatar} alt="Профиль" width={34} height={34} quality={100} className="object-cover w-full h-full" unoptimized />
        </Link>
      </div>
      ...
      <div className="w-full max-w-[346px] h-[44px] bg-[#E5E5EA] rounded-[22px] p-[4px] flex relative mb-[16px] shrink-0">
        ...
        />
        <button onClick={() => setOrderType('pickup')} className={`flex-1 flex justify-center items-center relative z-10 font-['Benzin'] text-[10px] font-extrabold uppercase transition-colors duration-300 ${orderType === 'pickup' ? 'text-[#FF008C]' : 'text-[#8E8E93]'}`}>Самовывоз</button>
        <button onClick={() => setOrderType('delivery')} className={`flex-1 flex justify-center items-center relative z-10 font-['Benzin'] text-[10px] font-extrabold uppercase transition-colors duration-300 ${orderType === 'delivery' ? 'text-[#FF008C]' : 'text-[#8E8E93]'}`}>Доставка</button>
      </div>
      ...
      {isLoading ? (
        <div className="w-full flex justify-center mt-10">
          <span className="text-[#FF008C] font-['Benzin'] animate-pulse text-[14px] uppercase tracking-wider">Загрузка меню...</span>
        </div>
      ) : items.length === 0 ? (
        <div className="w-full flex justify-center mt-10">
          <span className="text-[#8E8E93] font-['Benzin'] text-[12px] uppercase">Скоро добавим азиатские снеки 🥟</span>
        </div>
      ) : (
...
function Card({ d }: { d: any }) {
  ...
  const price = orderType === 'delivery' ? d.deliveryPrice : d.pickupPrice;
  const cartCount = items
    .filter((it: any) => it.cartItemId && it.cartItemId.startsWith(d.id + '-'))
    .reduce((s: number, it: any) => s + (it.quantity || 0), 0);
  return (
    <Link href={d.href} className="relative w-[170px] h-[188px] bg-white rounded-[25px] shadow-[0px_5px_5.7px_4px_rgba(255,0,140,0.25)] shrink-0 block active:scale-95 transition-transform duration-300">
      <div className="absolute top-0 left-0 w-[170px] h-[162px] rounded-t-[25px] overflow-hidden z-0 bg-[#F2F2F7]">
        <Image draggable={false} src={d.img} alt={d.name} fill className="object-cover" onError={(e: any) => { e.currentTarget.src = '/images/aziatka/placeholder.jpg'; }} />
      ...
      <div className="absolute bottom-[14px] left-[5px] w-[160px] h-[24px] bg-gradient-to-r from-[#FF00EE]/20 to-[#FF008C]/20 backdrop-blur-md border border-[#FFFFFF]/40 box-border rounded-full flex items-center justify-center shadow-[0px_4px_6px_2px_rgba(8,0,255,0.15)] z-10 px-2">
        <span className={`${d.name.length > 10 ? 'text-[7px]' : 'text-[10px]'} font-extrabold bg-gradient-to-r from-[#FF00EE] to-[#FF008C] bg-clip-text text-transparent uppercase tracking-wider leading-none whitespace-nowrap`}>
          {d.name}
        </span>
      </div>
